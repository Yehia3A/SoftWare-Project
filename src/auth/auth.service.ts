...
export class AuthService {
  ...
  async signIn(email: string, password: string): Promise<{ access_token: string; payload: { _id: string; email: string; role: string } }> {
    const user: UserDocument = await this.usersService.findByEmail(email) as UserDocument;
    if (!user) {
